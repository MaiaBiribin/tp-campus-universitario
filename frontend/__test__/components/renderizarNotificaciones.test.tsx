...
jest.mock("@/services/notificaciones",()=>({
...
describe("componente renderizacion de notificaciones",()=>{
    const mockNotificacion:Notificacion={
        id_notificacion:23,
          ...
          evento:{
            ...
            horaFin:"21:30",
            estado:"habilitado",
            aula:{id_aula:9,nombre:"205",capacidad:20,piso:2,ubicacion:"al fondo"},
            materia:{id_materia:10,nombre:"ingles",carrera:{id_carrera:4,nombre:"tecnicatura en sistemas"}},
            tipoEvento:{id_tipo_evento:40,nombre:"clase"}
    ...
    afterEach(()=>{
     ...
     it("deberia poder cargar las notificaciones",async()=>{
        (TraerTodasNotificaciones as jest.Mock).mockResolvedValue(new Promise(()=>{}))

         render(<RenderizarNotifiaciones/>)
         await waitFor(()=>{
             expect(screen.getByText("Cargando Notifiaciones")).toBeInTheDocument()
         })
     ...
     it("deberia poder renderizar la lista de notificaciones",async()=>{
         ...
         await waitFor(()=>{
               ...
               expect(screen.getByRole("heading",{level:3,name:/clase de ingles/i})).toBeInTheDocument()

         });
   
     })

     it("deberia poder marca una notificacion como leida",async()=>{
        jest.useFakeTimers();
        const usuario=userEvent.setup({ advanceTimers: jest.advanceTimersByTime });

        (TraerTodasNotificaciones as jest.Mock).mockResolvedValue([mockNotificacion]);

        const mockLeidaNotificacion={mockNotificacion,leida:true};

        (NotificacionLeida as jest.Mock).mockResolvedValue(mockLeidaNotificacion);

        render(<RenderizarNotifiaciones/>)
        const botonLeida= await screen.findByRole("button",{name:/Marcar leída/i})
        
        await usuario.click(botonLeida);
